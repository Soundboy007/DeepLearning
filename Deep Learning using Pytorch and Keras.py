...
lines_filepath = '/home/gautam-admin/Desktop/anandnlp/cornell_movie_dialogs_corpus/cornell movie-dialogs corpus/movie_lines.txt'
conv_filepath = '/home/gautam-admin/Desktop/anandnlp/cornell_movie_dialogs_corpus/cornell movie-dialogs corpus/movie_conversations.txt'
...
datafile = "/home/gautam-admin/Desktop/anandnlp/cornell_movie_dialogs_corpus/cornell movie-dialogs corpus/formatted_movie_lines.txt"
...
path = '/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/label/output'

files = []
for f in tqdm([f for f in glob.glob(path + "**/*.xmi", recursive=True)]):
    name = (f.split('/')[-1].split('.')[0])
    mydoc = minidom.parse(f)
    items = mydoc.getElementsByTagName('refsem:UmlsConcept')
    files.append([name] + list(set([(item.attributes['code'].value) for item in items if item.attributes['codingScheme'].value=="ICD10CM"]))+(list(set([(item.attributes['code'].value) for item in items if item.attributes['codingScheme'].value=="SNOMEDCT_US"]))))
    
print(len(files))
[files for files in files[0]]
...
path = '/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/img'
...
images.to_csv('/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/images.csv', index=False)
data.to_csv('/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/train.csv', index=False)
...
# train = data
train = pd.read_csv('/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/train.csv')
train.head()
...
model.add(Dropout(0.5))
model.add(Dense(1667, activation='sigmoid'))
...
model.fit(trainX, trainy, epochs=300, validation_data=(testX, testy), batch_size=64)


# In[ ]:


# Keeping track the freqency of tokens
new_dict = {}
for i in train.columns[2:]:
#     print(i)
    new_dict[i] = train[i].sum()
...
path = '/home/gautam-admin/Desktop/anandnlp/projectxray/Project/png_files_sample/img/'+train['Id'][testy.sample().index[0]]+'.png'
img = cv2.imread(path)
...
top_3 = np.argsort(proba[0])[:-(num_classes+1):-1]
for i in range(num_classes):
    if proba[0][top_3[i]]>0.6 and new_dict[classes[top_3[i]]]<700:
...
[sorted(i[1:]) for i in new_words if i[0]==path.split('/')[-1].split('.')[0]][0]
